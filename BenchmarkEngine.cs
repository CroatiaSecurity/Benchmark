...
public class BenchmarkEngine
{
    ...
    private static readonly Dictionary<string, double> ReferenceScores = new()
    {
        ["CPU"] = 36000000.0,
        ["Memory"] = 2030000.0,
        ["Disk"] = 3600.0,
    ...
    public async Task<BenchmarkResults> RunAsync(CancellationToken ct = default)
    {
        ...
        await Task.Run(() => RunCpuBenchmark(results), ct);
        ct.ThrowIfCancellationRequested();

        Report("Running Memory benchmark...");
        await Task.Run(() => RunMemoryBenchmark(results), ct);
        ct.ThrowIfCancellationRequested();

        Report("Running Disk benchmark...");
        await Task.Run(() => RunDiskBenchmark(results), ct);
        ct.ThrowIfCancellationRequested();

        Report("Running GPU benchmark...");
        await Task.Run(() => RunGpuBenchmark(results), ct);
        ct.ThrowIfCancellationRequested();

        Report("Running Network benchmark...");
        await Task.Run(() => RunNetworkBenchmark(results), ct);
        ct.ThrowIfCancellationRequested();
        ...
        results.SystemAnalysis = await Task.Run(() => analyzer.Run(ct), ct);
    ...
    private static string GetOsName()
    {
        try
        {
            ...
            return first?["Caption"]?.ToString() ?? "Windows";
        }
        catch { return "Windows"; }
    }

    private static void RunCpuBenchmark(BenchmarkResults r)
    {
        try
        {
            ...
            if (cpu != null)
            {
                r.CpuName = cpu["Name"]?.ToString()?.Trim() ?? "Unknown";
    ...
    private static void RunMemoryBenchmark(BenchmarkResults r)
    {
        try
        {
            using var searcher = new ManagementObjectSearcher("SELECT Capacity FROM Win32_PhysicalMemory");
            using var results = searcher.Get();
            long total = results.Cast<ManagementObject>().Sum(mo => Convert.ToInt64(mo["Capacity"]));
    ...
    private static void RunDiskBenchmark(BenchmarkResults r)
    {
        r.DiskDrive = "C:";
        try
        {
            ...
            if (disk != null)
                r.DiskDrive = disk["DeviceID"]?.ToString() ?? "C:";
        }
        catch { }
    ...
    private static void RunGpuBenchmark(BenchmarkResults r)
    {
        try
        {
            ...
            r.GpuName = gpu?["Name"]?.ToString() ?? "Unknown";
            ...
            try
            {
                ...
                if (classKey != null)
                {
                    foreach (string subName in classKey.GetSubKeyNames())
                    {
                        ...
                        if (desc != null && desc.Equals(r.GpuName, StringComparison.OrdinalIgnoreCase))
                        {
            ...
            if (vramBytes <= 0 && gpu?["AdapterRAM"] != null)
    ...
    private static void RunNetworkBenchmark(BenchmarkResults r)
    {
        ...
        avgLatency = successCount > 0 ? avgLatency / successCount : 100;
...
#pragma warning restore SYSLIB0014
    ...
    private static void CalculateOverall(BenchmarkResults r)
    {
    ...
    private static void CalculateBottleneck(BenchmarkResults r)
    {
