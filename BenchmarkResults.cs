...
public class BenchmarkResults
{
    ...
    public string BottleneckType { get; set; } = "None";
    ...
    private static double Cap(double p) => Math.Round(p, 1);
    ...
    public string GetJson()
    {
        ...
        if (SystemAnalysis != null)
        {
            sb.AppendFormat(c, "  \"systemAnalysis\": {{\r\n");
...
.good{color:#22d3ee;} .warning{color:#fbbf24;} .critical{color:#ef4444;} .minor{color:#a78bfa;}
        ...
        if (SystemAnalysis != null)
        {
            string healthClass = SystemAnalysis.OverallHealth switch
            {
                "Good" => "good",
                "Warning" => "warning",
                "Critical" => "critical",
